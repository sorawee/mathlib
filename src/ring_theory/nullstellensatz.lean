...
begin
  rw ring_hom.injective_iff,
  intros x hx,
  rw ring_hom.comp_apply at hx,
  rw quotient.eq_zero_iff_mem at hx,
  refine classical.by_contradiction (λ hx0, _),
  refine absurd _ hI,
  rw eq_top_iff_one,
...
lemma map_poly_eq {S : Type*} [comm_ring S] (ϕ : mv_polynomial σ k →+* S) (p : mv_polynomial σ k) :
...
begin
  ...
  let ϕ : k →+* I.quotient := (ideal.quotient.mk I).comp mv_polynomial.C,
  -- Sorry is from other PR using Jacobson rings
  have hϕ : function.bijective ϕ := ⟨quotient_mk_comp_C_injective I hI.1, sorry⟩,
  obtain ⟨φ, hφ⟩ := function.surjective.has_right_inverse hϕ.2,

  let x : σ → k := λ s, φ ((ideal.quotient.mk I) (mv_polynomial.X s)),
  have hx : ∀ s : σ, ϕ (x s) = (ideal.quotient.mk I) (mv_polynomial.X s) :=
    λ s, hφ ((ideal.quotient.mk I) (mv_polynomial.X s)),
  refine ⟨x, _⟩,
  have : I' {x} ≤ I,
  { intros p hp,
    rw [← quotient.eq_zero_iff_mem, map_poly_eq (ideal.quotient.mk I) p, mv_polynomial.eval₂_eq'],
    rw [← lemma1, mv_polynomial.eval_eq'] at hp,
    replace hp := congr_arg ϕ hp,
    rw [ϕ.map_zero, ϕ.map_sum] at hp,
    refine trans _ hp,
    congr,
    ext,
    rw [ϕ.map_mul, ϕ.map_prod],
    congr,
    ext s,
    rw [ϕ.map_pow, hx s] },
